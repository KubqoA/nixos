...
in rec {
  /* Reads the contents of the given ‹dir› and executes ‹fn› on ‹*.nix› files,
     and executes ‹dirfn› for directories. The results are stored as
     ‹nameValuePair› with the files/directories name as key. Serves as a helper
     function for ‹mapModules› and ‹mapModulesRec›. If the value returned by
     one of the functions is ‹null›, the key-value pair is discarded.
